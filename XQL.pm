...
#
# Same as in XML::DOM, these should be moved to a seperate module...
#
    ...
    my $ChIdeographic = '(?:\xE3\x80[\x87\xA1-\xA9]|\xE4(?:[\xB8-\xBF][\x80-\xBF])|\xE5(?:[\x80-\xBF][\x80-\xBF])|\xE6(?:[\x80-\xBF][\x80-\xBF])|\xE7(?:[\x80-\xBF][\x80-\xBF])|\xE8(?:[\x80-\xBF][\x80-\xBF])|\xE9(?:[\x80-\xBD][\x80-\xBF]|\xBE[\x80-\xA5]))';

    my $ChDigit = '(?:[0-9]|\xD9[\xA0-\xA9]|\xDB[\xB0-\xB9]|\xE0(?:\xA5[\xA6-\xAF]|\xA7[\xA6-\xAF]|\xA9[\xA6-\xAF]|\xAB[\xA6-\xAF]|\xAD[\xA6-\xAF]|\xAF[\xA7-\xAF]|\xB1[\xA6-\xAF]|\xB3[\xA6-\xAF]|\xB5[\xA6-\xAF]|\xB9[\x90-\x99]|\xBB[\x90-\x99]|\xBC[\xA0-\xA9]))';

    my $ChExtender = '(?:\xC2\xB7|\xCB[\x90\x91]|\xCE\x87|\xD9\x80|\xE0(?:\xB9\x86|\xBB\x86)|\xE3(?:\x80[\x85\xB1-\xB5]|\x82[\x9D\x9E]|\x83[\xBC-\xBE]))';
    ...
    my $ChLetter = "(?:$ChBaseChar|$ChIdeographic)";
    my $ChNameChar = "(?:[-._:]|$ChLetter|$ChDigit|$ChCombiningChar|$ChExtender)";
...
sub parse_q
{
    ...
    while ($str =~ /^([^$d1$d2]*)($d1|($d2))/)
    {
	defined ($3) and return ($', $match . $1);		# $d2 found

	# match delimiters recursively
	$match .= $1 . $2;

	($str, $found) = parse_q ($qname, $q, $', $d1, $d2);
	$match .= $found . $d2;
    }
...
sub Lexer 
{
	...
	s/^($ReName)//o			and return ('NCName', $1);
    }
}
...
See 'XML::XQL Global functions' for details.
...
for the primitive XQL types or the Date class (in Date.pm.)
