...
The XML::XQL module has added functionality to the XQL spec, called I<XQL+>.
...
In effect, the '//' can be though of as a substitute for one or more levels of 
...
Numbers (integers and floats) are wrapped in an XML::XQL::Number object and
string in a XML::XQL::Text object. Booleans (as returned by true() and false())
are wrapped in an XML::XQL::Boolean objects.
...
forget the quotes! (Or use the q// or qq// delimiters in XQL+, see XML::XQL
man page.)
...
its Element descendants. See XML::XQL man page for details.
...
rawText() of its Element descendants. See XML::XQL man page for details.

=item Method: value()

Returns a type cast version of the value of a node. If no datat type is
...
See the XML::DOM man page for node type values of nodes not mentioned here.
...
almost any imaginable format. See the XML::XQL man page to plug in your own
Date implementation.
...
See XML::XQL man page for details.
